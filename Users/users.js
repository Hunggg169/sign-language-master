...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    const apiUrl = "http://localhost/Sign_Language_Master/Users/users.php";

    if (!userList) {
        console.error("Không tìm thấy phần tử #userList trong HTML!");
        return;
    }
    ...
    try {
        ...
        if (data.status === "success" && Array.isArray(data.users)) {
            ...
            data.users.forEach((user) => {
                const avatarPath = user.avatar ? user.avatar : "../Image/avatars.png";

                const userCard = document.createElement("div");
                userCard.className = "user-card";
                userCard.innerHTML = `
          <img src="${avatarPath}" alt="Avatar người dùng" class="avatar">
          <h3>${user.username}</h3>
        ...
        `;
            ...
            userList.innerHTML = `<p>Không có người dùng nào.</p>`;
        }
    } catch (error) {
        console.error("Lỗi:", error);
        userList.innerHTML = `<p>Không thể tải dữ liệu người dùng.</p>`;
    }
});