...
document.addEventListener("DOMContentLoaded", async() => {
    ...
    try {
        const res = await fetch(
            "http://localhost/Sign_Language_Master/Settings/email.php"
        );
        if (!res.ok) throw new Error("Không thể gọi PHP API.");

        const data = await res.json();

        if (data.status === "success") {
            usernameInput.value = data.user.name || data.user.username;
            emailInput.value = data.user.email;
        } else {
            usernameInput.value = "";
            emailInput.value = "";
            alert("Không lấy được thông tin: " + data.message);
        }
    } catch (error) {
        console.error("Lỗi fetch:", error);
        alert("Không thể kết nối máy chủ hoặc JSON sai.");
    }
});