...
document.addEventListener("DOMContentLoaded", function() {
    ...
    //  Reset tất cả input khi tải lại trang
    form.reset();
    current.value = "";
    newPass.value = "";
    confirm.value = "";
    ...
    form.addEventListener("submit", async(e) => {
        ...
        // Nếu nhập lại không khớp
        if (newVal !== confirmVal) {
            messageBox.textContent = "Mật khẩu nhập lại không khớp.";
            messageBox.style.color = "red";
            return;
        }

        // Giả lập thành công (tự động hiển thị thành công)
        messageBox.textContent = "Đổi mật khẩu thành công!";
